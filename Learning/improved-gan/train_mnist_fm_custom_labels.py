...
trainx = trainx[0:9000]
trainx_unl = trainx.copy()
trainx_unl2 = trainx.copy()
trainy = np.load(args.labels)
trainy = trainy[0:9000]
trainy_true = data['y_test'].astype(np.int32)[:9000]
#trainy_true = data['y_test'].astype(np.int32)[:1000]
nr_batches_train = int(trainx.shape[0]/args.batch_size)
testx = data['x_test'].astype(th.config.floatX)[9000:]
testy = data['y_test'].astype(np.int32)[9000:]
nr_batches_test = int(testx.shape[0]/args.batch_size)

# select labeled data
inds = data_rng.permutation(trainx.shape[0])
#inds = data_rng.permutation(1000)
