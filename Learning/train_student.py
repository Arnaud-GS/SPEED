...
tf.flags.DEFINE_integer('teachers_max_steps', 5000,
...
tf.flags.DEFINE_float('lap_scale', 0.31,
...
tf.flags.DEFINE_boolean('only_get_ohe', False, 'Activate deeper CNN model')
...
def prepare_student_data(dataset, nb_teachers, save=False):
  ...
  filepath_logits = FLAGS.data_dir + "/" + str(dataset) + '_' + str(nb_teachers) + '_teacherspreds_.npy' #NOLINT(long-line)
	
  print(teachers_preds.shape)
    # Dump student noisy labels array
  with tf.gfile.Open(filepath_logits, mode='w') as file_obj:
  ...
  if save:
    ...
    if FLAGS.distributed_noise:
      if FLAGS.nb_successful_teachers == None:
        filepath = FLAGS.data_dir + "/" + str(dataset) + '_' + str(nb_teachers) + '_student_labels_distributed_noise_lap_' + str(FLAGS.lap_scale) + '_nb_st_' + str(nb_teachers) + '.npy' #NOLINT(long-line)
      else:
        filepath = FLAGS.data_dir + "/" + str(dataset) + '_' + str(nb_teachers) + '_student_labels_distributed_noise_lap_' + str(FLAGS.lap_scale) + '_nb_st_' + str(FLAGS.nb_successful_teachers) + '.npy' #NOLINT(long-line)
    else:
...
#  if train_student was launched only to generate the .npy files, it can be stopped now
  if FLAGS.only_get_ohe:
    return False
